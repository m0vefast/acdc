...
These tests use `libghostty-vt`, whose native build requires Zig 0.15.x.
...
If Zig is not installed, install a 0.15.x release with your package manager or
from https://ziglang.org/download/[ziglang.org/download]. During the first
build, `libghostty-vt-sys` fetches Ghostty at its pinned commit and compiles the
virtual terminal library with Zig.
