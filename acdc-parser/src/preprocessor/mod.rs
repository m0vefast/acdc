...
impl Preprocessor {
    ...
    fn handle_include_result(include_result: IncludeResult, state: &mut PreprocessorState) {
        ...
        // Advance the running offset and splice the expanded lines in. Line/
        // column drift from the removed directive line is reconciled downstream
        // via the location remap adopted from upstream (see lib.rs), which
        // superseded the old per-directive `include_expansions` recording
        // (dropped — it would double-compensate against the remap).
