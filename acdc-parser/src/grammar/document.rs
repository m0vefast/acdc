...
fn parse_table_block_impl<'input>(
        ...
        block_metadata.metadata.attributes.get("cols")
    {
        ...
        if let Some(n) = bare_count {
            // `[cols=N]` materializes N `ColumnFormat`s eagerly and the pad model
            // materializes up to N empty cells per incomplete row, so an absurd N
            // (typo, or a hostile pasted `.adoc` carrying `[cols=100000000]`) would
            // allocate gigabytes before a single body row is read. This branch used
            // to clamp silently to a Glyph-local cap of 1000; it now goes through
            // upstream's bound and REJECTS, which is both stricter and consistent
            // with the `N*` spelling one branch down — `[cols="101*"]` already
            // errors, so `[cols=101]` erroring too is the non-surprising behaviour.
            if n > MAX_TABLE_COLUMNS {
