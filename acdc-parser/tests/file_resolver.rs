...
fn cyclic_include_bounded_by_depth_limit_no_stack_overflow() {
    ...
    // Glyph's own `MAX_INCLUDE_DEPTH` was dropped in favour of upstream's
    // `max-include-depth` document attribute (same default of 64, but
    // Asciidoctor-configurable), so the invariant under test is unchanged —
    // the cycle is bounded, the parse returns, and a warning reaches the
    // consumer — while the message is now upstream's.
    let depth_warning = result
        .warnings()
        .iter()
        .any(|w| w.kind.to_string().contains("maximum include depth"));
    assert!(
        depth_warning,
