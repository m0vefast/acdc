...
#[test]
fn absurd_bare_cols_count_is_rejected_no_oom() {
    ...
    // cells — gigabytes, or a crash, on a pasted or mistyped `.adoc`.
    //
    // This test used to require the parse to SUCCEED with the width silently
    // clamped to a Glyph-local cap of 1000. Upstream has since bounded tables
    // itself and REJECTS the equivalent `[cols="101*"]` spelling outright
    // (`table_resource_limits.rs`), so clamping `[cols=101]` while erroring on
    // `[cols="101*"]` would make two spellings of one thing behave differently.
    // The guard being tested — no unbounded allocation — holds either way; only
    // the recovery differs, and rejecting is the more honest of the two: a
    // silently-resized table is a document the author never wrote.
    let opts = Options::builder().with_safe_mode(SafeMode::Unsafe).build();
    for src in [
        "[cols=100000000]\n|===\n|a |b\n|===\n",
        "[cols=\"100000000*\"]\n|===\n|a |b\n|===\n",
    ] {
        let Err(error) = parse(src, &opts) else {
            panic!("absurd column count must be rejected, not clamped. src={src:?}");
        };
        let acdc_parser::Error::Parse(_, message) = error else {
            panic!("expected a parse error carrying the limit message. src={src:?}");
        };
        assert!(
            message.contains("column count") && message.contains("exceeds the maximum"),
            "message must name the bound that was exceeded, got {message:?}"
        );
    }
}
