...
// --- Inline anchor disabled inside constrained monospace -------------
//
// AsciiDoc spec: constrained monospace's substitution group is
// `[specialchars, callouts]` — `macros` (which includes inline anchors
// `[[id]]`) is NOT in scope. `` `[[my-id]]` `` should render as literal
// `[[my-id]]` inside a `<code>` span, NOT spawn an `inline-anchor` node.

#[test]
fn anchor_disabled_inside_constrained_monospace() {
    let json = parse_json("X `[[anchor-id]]` Y");
    // Must NOT emit an Anchor node — JSON shape is {"name":"anchor","type":"inline",...}.
    assert!(
        !json.contains(r##""name":"anchor""##),
        "anchor incorrectly emitted inside constrained monospace; {json}"
    );
    // And the monospace span itself must be present (content preserved literal).
    assert!(
        json.contains(r##""variant":"code""##),
        "monospace span missing despite anchor disable; {json}"
    );
}
